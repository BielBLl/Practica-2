...
Una vez visualizados los datos, vamos a realizar un test para analizar si realmente los maestro de infantil tienen una mayor tendencia a tener problemas en la voz. 
Entonces, el contraste a realizar seria el siguiente:

$$ \begin{cases} H_0: p_{infantil} = p_{otros} \\ H_1: p_{infantil}> p_{otros} \end{cases} $$
Para ello, realizaremos el test exacto de Fisher, el cual solo necesitamos que la muestra sea independiente para aplicarlo.
...
```
Observamos un p-valor de 0.364, lo cual es mayor al nivel de significancia, 0.05. Esto nos indica que no hay indicios suficientes para rechazar la hipotesis nula, es decir, de que la proporcion de maestros de infantil con problemas vocales sea mayor a la de los otros docentes.
