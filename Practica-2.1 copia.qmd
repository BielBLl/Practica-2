...
Al tener ya los datos limpios y homogeneizados podemos pasar a la pregunta particular que le ha tocado a nuestro grupo, la pregunta 2, "¿Hay mayor incidencia de síntomas vocales en maestras de infantil?".

```{r, include=FALSE}
#nuevo dataset con una columna que nos indica el cuerpo docente y otra con el diagnostico
sub_data=data.frame(data$cuerpo_docente,data$diagnostico_especialista)
#miramos cuantos valores unicos hay en la columna de cuerpo docente
...
sub_data_limpia<-sub_data %>% 
  mutate(across(everything(),~case_when(
     ...
     . %in% c("Secundaria i Batxillerat","Educació Secundària") ~ "Secundaria/Bachiller",
     . == "Educació Infantil"~"Infantil",
     . %in% c("Universitat","Universidad")~"Universidad",
     . %in% c("Professor de saxòfon en el Conservatori Professional de Música ","Música i arts escèniques","conservatori","Arts escèniques (Conservatorio, dansa)","Conservatori","Música i Arts escèniques. Dansa Clàssica.","Música i arts escèniques ","Conservatori professional ","Música i Arts Escèniques ")~"Música/Artes Escénicas",
     . %in% c("Règim Especial","Règim Especial (Secundària)","UEECO ")~"Regimen Especial",
     . %in% c("Educació Primària","Ed Primària, PT ( Pedagogia Terapèutica) .faig feina a Institut")~"Primaria",
     TRUE~.
   )))

colnames(sub_data_limpia)=c("cuerpo_docente","diagnostico_especialista")
unique(sub_data_limpia$cuerpo_docente)
glimpse(sub_data_limpia)
```
...
ggplot(taula, aes(fill = diagnostico_especialista, y = Freq, x = cuerpo_docente)) +
  ...
  scale_fill_manual(values = c("No" = "#FF83FA", "No ho record" = "#C1FFC1", "Si" = "#3357FF"))+
  ggtitle("Distribución de Diagnósticos por Cuerpos Docentes")
```

Ahora, añadiremos una tabla de proporciones que nos indica la proporción de Si, No, No ho record de cada cuerpo docente.

```{r, include=FALSE}
# Crear la tabla con el conteo de problemas
numero_problemas <- sub_data_limpia %>%
  group_by(cuerpo_docente) %>%
  summarise(
    total_docentes = n(), 
    problemas_si = sum(diagnostico_especialista == "Si", na.rm = TRUE),  # Cantidad que respondió "sí"
    problemas_ns = sum(diagnostico_especialista == "No ho record", na.rm = TRUE),  # Cantidad que respondió "No ho record"
    problemas_no = sum(diagnostico_especialista == "No", na.rm = TRUE)   # Cantidad que respondió "no"
  )

# Crear una nueva tabla con las proporciones
proporciones_problemas <- numero_problemas %>%
  mutate(
    prop_si = round(problemas_si / total_docentes, 3),
    prop_ns = round(problemas_ns / total_docentes, 3),
    prop_no = round(problemas_no / total_docentes, 3)
  )
proporciones_problemas <- proporciones_problemas %>%
  select(cuerpo_docente, prop_si, prop_ns, prop_no)

names(proporciones_problemas) <- c("CuerpoDocente","Proporcion:Si", "Proprcion: No ho record", "Proporcion:No")
...
tabla_bonita <- proporciones_problemas %>%
  gt() %>%
  tab_header(
    title = "Proporciones de Problemas por Cuerpo Docente",
    subtitle = "Análisis de diagnóstico del especialista"
  ) %>%
  fmt_percent(
    columns = c("Proporcion:Si", "Proprcion: No ho record", "Proporcion:No"),
    decimals = 1
  ) %>%
  cols_label(
    CuerpoDocente = "Cuerpo Docente",
    `Proporcion:Si` = "Sí",
    `Proprcion: No ho record` = "No Record",
    `Proporcion:No` = "No"
  ) %>%
  tab_options(
...
De esta tabla podemos observar como los maestros de infantil junto con los de régimen especial son los que más diagnósticos de lesiones en las cuerdas vocales han recibido, por lo que podria significar una respuesta afirmativa ante la pregunta que nos ha sido planteada. Para estudiar esto con más detalle se podria hacer alguna inferencia donde comparamos estos valores con la probabilidad de par¡decer un problema, pero con la muestra que nos ha sido dada los resultados no serían fiables por la poca densidad muestral.

Una vez visualizados los datos, vamos a realizar un test para analizar si realmente los maestro de infantil tienen una mayor tendencia a tener problemas en la voz. Entonces, el contraste a realizar seria el siguiente:

$$ \begin{cases} H_0: p_{infantil} = p_{otros} \\ H_1: p_{infantil}> p_{otros} \end{cases} $$ Para ello, realizaremos el test exacto de Fisher, el cual solo necesitamos que la muestra sea independiente para aplicarlo.

```{r, include=FALSE}
# Crear la tabla de contingencia
tabla <- matrix(c(3, 30, 9, 143), nrow = 2)

# Realizar el test de Fisher
resultado_fisher <- fisher.test(tabla, alternative = "greater")

# Ver el resultado
print(resultado_fisher)
```

Observamos un p-valor de 0.364, lo cual es mayor al nivel de significancia, 0.05. Esto nos indica que no hay indicios suficientes para rechazar la hipótesis nula, es decir, que la proporción de maestros de infantil con problemas vocales puede que sea mayor a la de los otros docentes.
